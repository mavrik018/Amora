...
final connectionRequestsStreamProvider = StreamProvider<List<ConnectionRequest>>((ref) {
  final repo = ref.watch(connectionProvider);
  return repo.getConnectionRequestsStream();
});

final acceptedConnectionsStreamProvider = StreamProvider<List<ConnectionRequest>>((ref) {
  final repo = ref.watch(connectionProvider);
...
class ConnectionRepository {
  ...
  Stream<List<ConnectionRequest>> getConnectionRequestsStream() {
    ...
    return _supabase
        ...
        .map((data) => data.where((item) => item['receiver_id'] == myId && item['status'] == 'pending').toList())
        .asyncMap((data) async {
          // Fetch sender profiles manually for each request
          List<ConnectionRequest> requests = [];
          for (var item in data) {
            final senderId = item['sender_id'];
            final profileRes = await _supabase.from('profiles').select().eq('id', senderId).single();
    ...
    return _supabase
        ...
        .asyncMap((requests) async {
          ...
          for (var req in requests) {
            final otherId = req.senderId == myId ? req.receiverId : req.senderId;
            final profileRes = await _supabase.from('profiles').select().eq('id', otherId).single();
            populated.add(ConnectionRequest(
              id: req.id,
              senderId: req.senderId,
              receiverId: req.receiverId,
              status: req.status,
              createdAt: req.createdAt,
              sender: req.senderId == otherId ? ProfileModel.fromJson(profileRes) : null,
              receiver: req.receiverId == otherId ? ProfileModel.fromJson(profileRes) : null,
            ));
          }
          return populated;
        });
  ...
  Future<void> sendConnectionRequest(String receiverId) async {
    ...
    final existing = await _supabase.from('connections').select().or('and(sender_id.eq.$myId,receiver_id.eq.$receiverId),and(sender_id.eq.$receiverId,receiver_id.eq.$myId)');
